...
**Dự án**: E-Commerce Full-stack Web (Backend: Node.js / Express + TypeScript + MongoDB + Cloudinary)  
...
- Mã nguồn kiểm thử: `be/src/tests/product.test.ts`
- Mã nguồn nghiệp vụ: `be/src/controllers/product.controller.ts`, `be/src/schemas/product.schema.ts`, `be/src/routes/product.route.ts`, `be/src/models/product.model.ts`, `be/src/config/cloudinary.ts`, `be/src/middleware/validate.ts`
...
Dựa trên đặc tả validation schema tại [`product.schema.ts`](file:///d:/admin/e-commerce-web/be/src/schemas/product.schema.ts) và logic nghiệp vụ xử lý danh mục sản phẩm tại [`product.controller.ts`](file:///d:/admin/e-commerce-web/be/src/controllers/product.controller.ts), các tham số đầu vào được phân tích thành các phân vùng tương đương hợp lệ (Valid EP), không hợp lệ (Invalid EP) và các ngưỡng giá trị biên (Robustness BVA).

#### A. Bảng phân tích Phân vùng tương đương (EP) cho các tham số Product Module
...
#### B. Phân tích Giá trị biên (Boundary Value Analysis - BVA) cho Giá sản phẩm (`price`)

Theo lý thuyết BVA mở rộng (Robustness BVA: $6n + 1$) từ Chương 4, với biến số nguyên `price` có miền hợp lệ $[\text{Min}, \text{Max}] = [1,000; 1,000,000,000]$:

```
        Invalid (Min-)        Valid Domain [1,000 .. 1,000,000,000]           Invalid (Max+)
 <--------------------------|======================================|------------------------->
   ... 0    998    999      1,000   1,001     ... 25,000,000 ...   1,000,000,000  1,000,000,001
  [--- Too Low / Neg ---]   [Min]   [Min+]           [Nom]              [Max]     [--- Too High ---]
```

| Ngưỡng BVA | Giá trị kiểm thử | Payload mẫu | Kỳ vọng HTTP Status | Thông báo lỗi / Kết quả nhận được | Ý nghĩa kỹ thuật |
| :--- | :--- | :--- | :---: | :--- | :--- |
| **$\text{Min}^-$** | 999 VND | `{ price: 999, ... }` | **400** | `Price must be at least 1,000 VND` | Ngăn chặn giá sản phẩm quá thấp hoặc bằng 0 |
| **$\text{Min}$** | 1,000 VND | `{ price: 1000, ... }` | **200** | `{ success: true, product: { price: 1000 } }` | Giá tối thiểu hợp lệ theo đồng tiền VND |
| **$\text{Min}^+$** | 1,001 VND | `{ price: 1001, ... }` | **200** | Chấp nhận giá trị | Biên trên cận dưới |
| **$\text{Nom}$** | 25,000,000 VND | `{ price: 25000000, ... }` | **200** | Chấp nhận giá trị thông dụng | Giá trị điển hình (Laptop Gaming) |
| **$\text{Max}$** | 1,000,000,000 VND | `{ price: 1000000000, ... }` | **200** | Chấp nhận giá trị cận trên | Giới hạn 1 tỷ đồng cho hàng hóa thương mại |
| **$\text{Max}^+$** | 1,000,000,001 VND | `{ price: 1000000001, ... }` | **400** | `Price must be at most 1,000,000,000 VND` | Chặn tràn số hoặc định giá phi thực tế |
| **Float Boundary** | 1,500.5 VND | `{ price: 1500.5, ... }` | **400** | `Price must be an integer` | Zod `.int()` ngăn số lẻ hào/xu |
| **Coercion Boundary** | `"5000"` (String) | `{ price: "5000", ... }` | **200** | `product.price === 5000` (Number) | `z.coerce.number()` tự chuyển kiểu chuỗi sang số |
...
| 7 | **`if (product.public_id)`** (L258) | **Có ảnh Cloudinary $\to$ Gọi `destroy`** | **Không có ảnh $\to$ Bỏ qua bước gọi API Cloud** | **TC_PROD_ADD_06** *(Có ảnh)* | **TC_PROD_ADD_07** *(Không ảnh)* |
...
1. **Điểm mù 1: Xác thực việc Hủy ảnh Cloudinary (Cloud Resource Cleanup Verification)**
   - *Hạn chế của Blackbox*: Khi gọi `DELETE /api/products/:id`, Blackbox chỉ nhận về HTTP `200 { success: true }`. Blackbox hoàn toàn không thể biết liệu file ảnh trên Cloudinary CDN có thực sự bị xóa hay vẫn đang âm thầm ngốn dung lượng tài khoản của doanh nghiệp.
   - *Cách Whitebox giải quyết*: Sử dụng Jest Spy trên module `config/cloudinary`:
     ```typescript
     expect(cloudinary.uploader.destroy).toHaveBeenCalledWith("sample_public_id");
     ```
...
2. **Điểm mù 2: Kiểm thử Phân trang Cursor hiệu năng cao trong bộ nhớ**
   - *Hạn chế của Blackbox*: Để kiểm thử `skip` và `limit` hoạt động đúng, Blackbox phải kết nối CSDL thật và seed hàng trăm bản ghi, gây chậm chạp cho CI/CD pipeline.
   - *Cách Whitebox giải quyết*: Dùng Chaining Mock Cursor trong `beforeEach`:
     ```typescript
     mockCursor = {
