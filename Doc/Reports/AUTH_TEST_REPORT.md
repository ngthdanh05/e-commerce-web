...
**Tài liệu tham chiếu**: 
- `Chuong 4 - Cac Ky Thuat Thiet Ke Test.pdf` (Kỹ thuật phân hoạch tương đương EP, phân tích giá trị biên BVA, đồ thị dòng điều khiển CFG, độ phức tạp Cyclomatic $V(G)$, độ bao phủ câu lệnh & nhánh)
- Mã nguồn kiểm thử: `be/src/tests/auth.test.ts`
- Mã nguồn nghiệp vụ: `be/src/controllers/user.controller.ts`, `be/src/schemas/auth.schema.ts`, `be/src/routes/auth.route.ts`, `be/src/routes/admin.route.ts`, `be/src/middleware/auth.ts`, `be/src/middleware/validate.ts`
...
Dựa trên đặc tả validation schema tại [`auth.schema.ts`](file:///d:/admin/e-commerce-web/be/src/schemas/auth.schema.ts) và logic xử lý nghiệp vụ tại [`user.controller.ts`](file:///d:/admin/e-commerce-web/be/src/controllers/user.controller.ts), toàn bộ tham số đầu vào được phân rã thành các lớp tương đương hợp lệ (Valid Partitions), không hợp lệ (Invalid Partitions) và xác định các điểm chuyển tiếp biên (Boundaries) theo chuẩn giáo trình Kiểm thử phần mềm (Standard BVA & Robustness BVA: $\text{Min}^-$, $\text{Min}$, $\text{Min}^+$, $\text{Nom}$, $\text{Max}^-$, $\text{Max}$, $\text{Max}^+$).

#### Bảng 1.1: Phân tích Phân vùng tương đương (EP) cho các tham số Auth

| Tham số (Input) | Miền giá trị / Ràng buộc đặc tả | Lớp tương đương hợp lệ (Valid EP) | Lớp tương đương không hợp lệ (Invalid EP) |
| :--- | :--- | :--- | :--- |
| **`name`** (Họ và tên) | Chuỗi ký tự, độ dài từ 2 đến 50 ký tự sau khi `.trim()`. | • **EP-V1**: Chuỗi ký tự độ dài $2 \le \text{len} \le 50$ (vd: `"Test User"`). | • **EP-I1**: Chuỗi rỗng `""` hoặc chỉ khoảng trắng `"   "` ($\text{len} = 0$).<br>• **EP-I2**: Độ dài $< 2$ ký tự (vd: `"A"`).<br>• **EP-I3**: Độ dài $> 50$ ký tự (vd: chuỗi 51 ký tự).<br>• **EP-I4**: Sai kiểu dữ liệu (`null`, `number`, `boolean`). |
| **`email`** (Thư điện tử) | Chuỗi định dạng email chuẩn RFC 5322 regex: `^[^\s@]+@[^\s@]+\.[^\s@]+$`, độ dài từ 5 đến 254 ký tự. Tự động `.trim().toLowerCase()`. | • **EP-V2**: Email đúng định dạng regex, độ dài $5 \le \text{len} \le 254$ chưa tồn tại trong CSDL.<br>• **EP-V3**: Email có khoảng trắng đầu/cuối hoặc viết hoa (hệ thống tự chuẩn hóa). | • **EP-I5**: Bỏ trống email hoặc rỗng `""` (`EMAIL_REQUIRED`).<br>• **EP-I6**: Độ dài $< 5$ ký tự (vd: `"a@b."`).<br>• **EP-I7**: Độ dài $> 254$ ký tự (`EMAIL_TOO_LONG`).<br>• **EP-I8**: Sai định dạng email (thiếu `@`, thiếu domain, chứa dấu cách giữa chừng).<br>• **EP-I9**: Email hợp lệ nhưng **đã tồn tại** trong CSDL (`ACCOUNT_ALREADY_EXISTS`). |
| **`password`** (Mật khẩu đăng ký) | Độ dài từ 8 đến 64 ký tự. Bắt buộc: $\ge 1$ chữ hoa (`[A-Z]`), $\ge 1$ chữ thường (`[a-z]`), $\ge 1$ chữ số (`[0-9]`), $\ge 1$ ký tự đặc biệt (`[^A-Za-z0-9]`). | • **EP-V4**: Độ dài $8 \le \text{len} \le 64$, thỏa mãn đủ 4 điều kiện ký tự phức tạp (vd: `"Password123!"`). | • **EP-I10**: Độ dài $< 8$ ký tự (vd: `"Pass1!"`).<br>• **EP-I11**: Độ dài $> 64$ ký tự.<br>• **EP-I12**: Thiếu chữ hoa (vd: `"password123!"`).<br>• **EP-I13**: Thiếu chữ thường (vd: `"PASSWORD123!"`).<br>• **EP-I14**: Thiếu chữ số (vd: `"Password!!!"`).<br>• **EP-I15**: Thiếu ký tự đặc biệt (vd: `"Password1234"`). |
| **`password`** (Mật khẩu đăng nhập) | Chuỗi ký tự độ dài $\ge 1$. | • **EP-V5**: Khớp với mật khẩu đã băm trong CSDL (`bcrypt.compare` = `true`). | • **EP-I16**: Rỗng `""`.<br>• **EP-I17**: Không khớp mật khẩu (`WRONG_PASSWORD`). |
| **`account_status`** (Trạng thái tài khoản) | Cờ trạng thái `isBlocked` (boolean) trong Document User MongoDB. | • **EP-V6**: Tài khoản hoạt động bình thường (`isBlocked: false` hoặc `undefined`). | • **EP-I18**: Tài khoản bị quản trị viên khóa (`isBlocked: true`). |
| **`user_id`** (Định danh ObjectId) | Chuỗi Hex 24 ký tự hợp lệ của MongoDB BSON ObjectId. | • **EP-V7**: Hex string 24 ký tự tồn tại trong collection users. | • **EP-I19**: Hex string 24 ký tự nhưng **không tồn tại** trong DB (404).<br>• **EP-I20**: Chuỗi sai định dạng BSON ObjectId (vd: `"123"`). |
| **`block`** (Payload Toggle Block) | Boolean (`true` / `false`). | • **EP-V8**: Giá trị kiểu boolean (`true` hoặc `false`). | • **EP-I21**: Giá trị sai kiểu dữ liệu (`"string"`, `null`). |

---

#### Bảng 1.2: Phân tích Giá trị biên (Boundary Value Analysis - BVA) cho các trường số lượng ký tự

Theo lý thuyết BVA mở rộng (Robustness BVA) trong tài liệu Chương 4: với một biến $x \in [\text{Min}, \text{Max}]$, số lượng kịch bản biên cần kiểm tra là $6n + 1$ gồm các điểm:
- $\text{Min}^-$ (Giá trị ngay dưới cận dưới - Không hợp lệ)
- $\text{Min}$ (Cận dưới nhỏ nhất - Hợp lệ)
- $\text{Min}^+$ (Giá trị ngay trên cận dưới - Hợp lệ)
- $\text{Nom}$ (Giá trị điển hình nằm trong khoảng - Hợp lệ)
- $\text{Max}^-$ (Giá trị ngay dưới cận trên - Hợp lệ)
- $\text{Max}$ (Cận trên lớn nhất - Hợp lệ)
- $\text{Max}^+$ (Giá trị ngay trên cận trên - Không hợp lệ)

| Trường kiểm thử | Ngưỡng đặc tả | Điểm biên ($\text{Min}^-$, $\text{Min}$, $\text{Min}^+$, $\text{Nom}$, $\text{Max}^-$, $\text{Max}$, $\text{Max}^+$) | Giá trị đại diện (Payload Data) | Kết quả kỳ vọng | Ghi chú kỹ thuật |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **`name`** (Độ dài chuỗi) | $[2, 50]$ | • $\text{Min}^- = 1$<br>• $\text{Min} = 2$<br>• $\text{Min}^+ = 3$<br>• $\text{Nom} = 25$<br>• $\text{Max}^- = 49$<br>• $\text{Max} = 50$<br>• $\text{Max}^+ = 51$ | • `"A"`<br>• `"An"`<br>• `"Anh"`<br>• `"Nguyen Van Test User QA Lead"`<br>• Chuỗi 49 ký tự `'A'.repeat(49)`<br>• Chuỗi 50 ký tự `'A'.repeat(50)`<br>• Chuỗi 51 ký tự `'A'.repeat(51)` | • 400 Bad Request<br>• 200 OK<br>• 200 OK<br>• 200 OK<br>• 200 OK<br>• 200 OK<br>• 400 Bad Request | Zod Schema: `min(2), max(50)` |
| **`email`** (Độ dài chuỗi) | $[5, 254]$ | • $\text{Min}^- = 4$<br>• $\text{Min} = 5$<br>• $\text{Min}^+ = 6$<br>• $\text{Nom} = 20$<br>• $\text{Max}^- = 253$<br>• $\text{Max} = 254$<br>• $\text{Max}^+ = 255$ | • `"a@b."` (4 ký tự)<br>• `"a@b.c"` (5 ký tự)<br>• `"ab@c.d"` (6 ký tự)<br>• `"valid.user@test.com"`<br>• `${'a'.repeat(241)}@test.com` (253)<br>• `${'a'.repeat(242)}@test.com` (254)<br>• `${'a'.repeat(243)}@test.com` (255) | • 400 Bad Request<br>• 200 OK<br>• 200 OK<br>• 200 OK<br>• 200 OK<br>• 200 OK<br>• 400 Bad Request | BVA kiểm tra ngưỡng Min 5 chars & Max 254 chars (RFC 5321 standard). |
| **`password`** (Độ dài chuỗi) | $[8, 64]$ | • $\text{Min}^- = 7$<br>• $\text{Min} = 8$<br>• $\text{Min}^+ = 9$<br>• $\text{Nom} = 12$<br>• $\text{Max}^- = 63$<br>• $\text{Max} = 64$<br>• $\text{Max}^+ = 65$ | • `"Pass1!a"` (7 ký tự)<br>• `"Pass12!a"` (8 ký tự)<br>• `"Pass123!a"` (9 ký tự)<br>• `"Password123!"`<br>• `"P1!" + "a".repeat(60)` (63 ký tự)<br>• `"P1!" + "a".repeat(61)` (64 ký tự)<br>• `"P1!" + "a".repeat(62)` (65 ký tự) | • 400 Bad Request<br>• 200 OK<br>• 200 OK<br>• 200 OK<br>• 200 OK<br>• 200 OK<br>• 400 Bad Request | BVA kiểm tra ngưỡng tối thiểu 8 ký tự an toàn và chặn DoS băm Bcrypt (> 64 chars). |
...
## 🟢 PHẦN 2: THỰC THI KIỂM THỬ VÀ MÃ NGUỒN (TEST IMPLEMENTATION)

### 1. Trích xuất trực tiếp mã nguồn kiểm thử Jest / Supertest từ `auth.test.ts`

Toàn bộ test suite được tổ chức theo chuẩn AAA (Arrange - Act - Assert) với Supertest để giả lập HTTP call vào Express pipeline:

#### Nhóm 1: Kiểm thử Đăng ký (`POST /api/auth/register`) - Sanitization, Bcrypt Hashing, BVA Min-, Duplicate Email

```typescript
// Trích xuất từ be/src/tests/auth.test.ts (Dòng 96 - Dòng 162)
...
#### Nhóm 2: Kiểm thử Đăng nhập (`POST /api/auth/login`) - Valid Login, Blocked Account Guard, Wrong Password

```typescript
// Trích xuất từ be/src/tests/auth.test.ts (Dòng 167 - Dòng 223)
...
#### Nhóm 3: Kiểm thử Đăng xuất & Quản trị User (`/api/auth/logout`, `/api/admin/users`)

```typescript
// Trích xuất từ be/src/tests/auth.test.ts (Dòng 228 - Dòng 267)
...
### 2. Phân tích chi tiết Kiến trúc Mocking trong `auth.test.ts`

Việc kiểm thử tự động ở cấp độ Unit & Integration Test của module Auth yêu cầu cô lập hoàn toàn môi trường (Isolate external dependencies) để đảm bảo tính tất định (determinism), tốc độ thực thi cao và không bị phụ thuộc vào MongoDB database instance thật hay thuật toán mã hóa tiêu tốn CPU.

```mermaid
flowchart TD
    subgraph TestSuite ["auth.test.ts (Supertest Pipeline)"]
        TestClient["Supertest Client"] --> ExpressApp["Express app.ts"]
    end

    subgraph MockLayer ["Jest Mocking Infrastructure"]
        ExpressApp -->|Bypasses Auth Check| MockMiddleware["Mocked middleware/auth.ts<br/>(inspects 'Bearer admin')"]
        ExpressApp -->|Replaces Crypto Math| MockBcrypt["Mocked bcryptjs<br/>(Instant Hash & Compare)"]
        ExpressApp -->|In-Memory DB Operations| MockMongo["Mocked userCollection<br/>(findOne, insertOne, updateOne...)"]
    end

    subgraph RealComponents ["Unmocked Core SUT (System Under Test)"]
        ExpressApp --> ZodValidate["Zod Middleware (validate.ts & auth.schema.ts)"]
        ExpressApp --> AuthController["user.controller.ts (register, login, logout...)"]
    end
```

#### A. Mocking Bcrypt (`bcryptjs`)
- **Mục đích**: Thuật toán Bcrypt tính toán hàm băm với Salt Rounds $\ge 10$ là hoạt động "CPU-bound", gây độ trễ từ 100ms - 300ms cho mỗi test case. Đồng thời, việc so sánh mật khẩu ngẫu nhiên trong môi trường test cần được điều khiển chính xác kết quả trả về (`true` hoặc `false`).
- **Triển khai trong mã nguồn**:
  ...
  ```
  Trong `beforeEach`:
  - `bcrypt.hash.mockResolvedValue(MOCK_HASH)`: Trả về chuỗi băm giả lập `$2a$10$MockHashedPasswordStringWithSalt1234567890`. Nhờ đó, bài test `TC-AUTH-02` có thể assert chính xác:
    ```typescript
    expect(bcrypt.hash).toHaveBeenCalledWith("ValidPassword123!", 10);
    ```
  - `bcrypt.compare.mockResolvedValue(true)`: Mặc định xác thực mật khẩu hợp lệ.
  - Trong test case `TC-AUTH-07` (Sai mật khẩu), Jest điều khiển luồng trả về `false` một lần duy nhất bằng:
    ```typescript
    (bcrypt.compare as jest.Mock).mockResolvedValueOnce(false);
    ```
    Giúp buộc controller kích hoạt nhánh `if (!isPasswordValid) return res.status(401)`.

#### B. Mocking Middleware Xác thực và Phân quyền (`middleware/auth`)
- **Mục đích**: `admin.route.ts` được bảo vệ bởi hai middleware: `verifyToken` và `isAdmin`. Nếu không mock, các test case quản trị viên (`TC-AUTH-09`, `TC-AUTH-10`, `TC-AUTH-11`) phải trải qua quy trình tạo JWT thật, ký bằng secret key, giải mã và gắn vào `req.user`.
- **Triển khai trong mã nguồn**:
  ```typescript
  ...
  ```
  Mock này mô phỏng cơ chế Role-Based Access Control (RBAC): khi gửi header `Authorization: Bearer mock_admin_token`, middleware tự động inject `role: "admin"` cho `req.user` và cho phép request đi thẳng vào controller.

#### C. Mocking MongoDB Collection (`userCollection`)
- **Mục đích**: Tránh việc cần một kết nối MongoDB daemon thật hoạt động khi chạy CI/CD pipeline, đồng thời có thể định hình kết quả truy vấn CSDL cho từng kịch bản nghiệp vụ.
- **Triển khai trong mã nguồn**:
  ```typescript
  ...
  ```
  Đối tượng `mockCollection` được khởi tạo lại trong `beforeEach` với các mock function chuỗi (Chaining Mock) hỗ trợ đầy đủ các hàm của MongoDB driver:
  - `findOne`: Trả về `null` (chưa tồn tại tài khoản cho `TC-AUTH-01`, `TC-AUTH-02`), trả về user object (đã tồn tại cho `TC-AUTH-04`, user hợp lệ cho `TC-AUTH-05`, user bị khóa `isBlocked: true` cho `TC-AUTH-06`).
  - `insertOne`: Trả về `{ acknowledged: true, insertedId: "mock_user_id" }`.
  - `find().sort().skip().limit().project().toArray()`: Hỗ trợ fluent query builder trả về danh sách phân trang cho `TC-AUTH-09`.
  - `countDocuments`: Trả về tổng số bản ghi (vd: `1`).
  - `updateOne`: Trả về `{ modifiedCount: 1 }` cho `TC-AUTH-10`.
  - `deleteOne`: Trả về `{ deletedCount: 1 }`.
...
| 10 | **TC-AUTH-ADD-01** *(Bổ sung)* | Bao phủ hàm `profile` (User lấy thông tin cá nhân) | `user.controller.ts`: L99-110 (`profile` thành công) |
| 11 | **TC-AUTH-ADD-02** *(Bổ sung)* | Bao phủ câu lệnh `deleteOne` thành công của hàm `deleteUser` | `user.controller.ts`: L196-201 (`deleteOne`, 200 OK) |
| 12 | **TC-AUTH-ADD-03** *(Bổ sung)* | Bao phủ câu lệnh `!user` khi đăng nhập email không tồn tại | `user.controller.ts`: L53-54 (`ACCOUNT_NOT_FOUND`) |
| 13 | **TC-AUTH-ADD-04** *(Bổ sung)* | Bao phủ câu lệnh `!user` trong `toggleBlockUser` | `user.controller.ts`: L164-165 (`USER_NOT_FOUND`) |
| 14 | **TC-AUTH-WHITEBOX-ERR** *(Bổ sung)* | Bao phủ các block `catch (error)` bằng cách mock DB reject | `user.controller.ts`: L37-38, L83-84, L93-94, L150-151, L181-182 |
...
| 2 | `if (!user)` trong `loginUser` | Không có user trong DB $\to$ Trả về 404 | User tồn tại $\to$ Tiếp tục kiểm tra khóa | **TC-AUTH-ADD-03** *(hoặc TC-AUTH-11)* | **TC-AUTH-05** |
...
1. **Điểm mù 1: Xác thực tham số an toàn của thuật toán mã hóa (Cryptographic Strength)**
   - *Hạn chế của Blackbox*: Kiểm thử hộp đen chỉ thấy HTTP Status `200` và `{ success: true }`. Hộp đen hoàn toàn không biết mật khẩu có thực sự được băm hay lưu trực tiếp dưới dạng Plain Text, và có sử dụng Salt Rounds đủ mạnh ($\ge 10$) hay không.
   - *Cách Whitebox giải quyết*: Dùng Jest Spy/Mock trên module `bcryptjs`:
     ```typescript
     expect(bcrypt.hash).toHaveBeenCalledWith("ValidPassword123!", 10);
     expect(mockCollection.insertOne).toHaveBeenCalledWith(
...
2. **Điểm mù 2: Kiểm thử các trạng thái nội tại phức tạp (Internal State Manipulation)**
   - *Hạn chế của Blackbox*: Để kiểm thử tài khoản bị khóa (`isBlocked: true`), nếu chỉ dùng hộp đen end-to-end, tester phải đăng ký tài khoản $\to$ chờ Admin duyệt khóa $\to$ mới tiến hành đăng nhập thử. Quy trình này cồng kềnh và dễ gãy (flaky).
   - *Cách Whitebox giải quyết*: Thông qua `mockCollection.findOne.mockResolvedValue({ isBlocked: true })` trong `TC-AUTH-06`, tester ngay lập tức đưa hệ thống vào trạng thái khóa để kiểm tra câu lệnh rẽ nhánh `if (user.isBlocked) return res.status(403)`.

3. **Điểm mù 3: Kích hoạt khối xử lý ngoại lệ (Exception & Error Handling - 500 Internal Server Error)**
   - *Hạn chế của Blackbox*: Trong điều kiện bình thường, không thể ép MongoDB Cluster bị ngắt kết nối hoặc ép RAM server cạn kiệt để test các khối `catch (error)`.
   - *Cách Whitebox giải quyết*: Bằng cách gọi `mockCollection.findOne.mockRejectedValue(new Error("MongoNetworkTimeout"))`, Whitebox dễ dàng phủ $100\%$ các dòng lệnh trong khối `catch` để xác minh server không bị crash và luôn trả về chuẩn mã lỗi 500.
...
---
*Báo cáo được khởi tạo và phê duyệt bởi Senior QA Automation & Test Architect Lead.*
