...
- Mã nguồn kiểm thử: `be/src/tests/cart.test.ts`
- Mã nguồn nghiệp vụ: `be/src/controllers/cart.controller.ts`, `be/src/schemas/cart.schema.ts`, `be/src/routes/cart.route.ts`, `be/src/models/cart.model.ts`, `be/src/models/product.model.ts`, `be/src/middleware/auth.ts`, `be/src/middleware/validate.ts`
...
Dựa trên đặc tả validation schema tại [`cart.schema.ts`](file:///d:/admin/e-commerce-web/be/src/schemas/cart.schema.ts) và logic nghiệp vụ xử lý giỏ hàng tại [`cart.controller.ts`](file:///d:/admin/e-commerce-web/be/src/controllers/cart.controller.ts), toàn bộ tham số đầu vào được phân rã thành các phân vùng tương đương hợp lệ (Valid EP), không hợp lệ (Invalid EP) và các điểm biên kiểm thử (BVA) theo lý thuyết kiểm thử phần mềm chuẩn tắc.
...
#### Bảng 1.2: Phân tích Giá trị biên (Boundary Value Analysis - BVA) cho tham số `quantity`

Theo tài liệu Chương 4 (Mục IV.3 - Robustness BVA: $6n + 1$), đối với biến số nguyên `quantity` khi thêm vào giỏ hàng có miền giá trị hợp lệ $[\text{Min}, \text{Max}] = [1, 99]$:
- $\text{Min}^-$ (Ngay dưới cận dưới): $0$ (Không hợp lệ - Reject)
- $\text{Min}$ (Cận dưới nhỏ nhất): $1$ (Hợp lệ - Accept)
- $\text{Min}^+$ (Ngay trên cận dưới): $2$ (Hợp lệ - Accept)
- $\text{Nom}$ (Giá trị danh định trung bình): $50$ (Hợp lệ - Accept)
- $\text{Max}^-$ (Ngay dưới cận trên): $98$ (Hợp lệ - Accept)
- $\text{Max}$ (Cận trên lớn nhất): $99$ (Hợp lệ - Accept)
- $\text{Max}^+$ (Ngay trên cận trên): $100$ (Không hợp lệ - Reject)
- *Biên cực đoan (Extreme / Negative)*: $-1, -10$ (Không hợp lệ - Reject)
- *Biên kiểu dữ liệu (Type Boundary)*: $1.5, 2.8$ (Float - Reject)

```
       Invalid (Min-)      Valid Boundary Domain [1 .. 99]          Invalid (Max+)
 <-----------------------|=========================================|------------------------>
   ... -10   -1     0    1    2          ... 50 ...          98   99   100   101 ...
  [---- Negative ----] [Min] [Min+]        [Nom]           [Max-][Max] [----- Too Large -----]
```

| Giá trị BVA | Điểm kiểm thử | Payload mẫu | Kỳ vọng HTTP Status | Phản hồi lỗi chi tiết / Hành vi hệ thống | Ghi chú kỹ thuật |
| :--- | :--- | :--- | :---: | :--- | :--- |
| **Negative (-)** | Cực đoan âm | `{ productId: "prod_123", quantity: -1 }` | **400** | `{ error: "VALIDATION_ERROR", details: [{ field: "quantity", message: "Quantity must be at least 1" }] }` | Zod `min(1)` kích hoạt chặn trước Controller |
| **$\text{Min}^-$** | 0 | `{ productId: "prod_123", quantity: 0 }` | **400** | `{ error: "VALIDATION_ERROR", details: [{ field: "quantity", message: "Quantity must be at least 1" }] }` | Ngưỡng 0 không được phép thêm vào giỏ |
| **$\text{Min}$** | 1 | `{ productId: "prod_123", quantity: 1 }` | **200** | `{ success: true, message: "Product added to cart" }` | Biên hợp lệ nhỏ nhất |
| **$\text{Min}^+$** | 2 | `{ productId: "prod_123", quantity: 2 }` | **200** | `{ success: true, message: "Product added to cart" }` | Biên trên nhỏ nhất |
| **$\text{Nom}$** | 50 | `{ productId: "prod_123", quantity: 50 }` | **200** | `{ success: true, message: "Product added to cart" }` | Giá trị thông thường |
| **$\text{Max}^-$** | 98 | `{ productId: "prod_123", quantity: 98 }` | **200** | `{ success: true, message: "Product added to cart" }` | Biên dưới lớn nhất |
| **$\text{Max}$** | 99 | `{ productId: "prod_123", quantity: 99 }` | **200** | `{ success: true, message: "Product added to cart" }` | Cận trên tối đa cho phép mua |
| **$\text{Max}^+$** | 100 | `{ productId: "prod_123", quantity: 100 }` | **400** | `{ error: "VALIDATION_ERROR", details: [{ field: "quantity", message: "Quantity must not exceed 99" }] }` | Chặn hành vi đầu cơ hoặc tràn số |
| **Float** | 1.5 | `{ productId: "prod_123", quantity: 1.5 }` | **400** | `{ error: "VALIDATION_ERROR", details: [{ field: "quantity", message: "Invalid input: expected int, received number" }] }` | Zod `.int()` ngăn chặn số lượng hàng hóa phân mảnh |
...
```
*(Trong khi sản phẩm `prod_123` trên CSDL có giá niêm yết là 500,000 VND. Nếu hệ thống tin tưởng Client, tổng tiền giỏ hàng sẽ chỉ còn 20 VND thay vì 1,000,000 VND)*.
...
3. **Lớp 3 - Hàm tính toán lại tổng tiền độc lập (`recalculateCartTotal`)**:
   - Tại [`cart.controller.ts`](file:///d:/admin/e-commerce-web/be/src/controllers/cart.controller.ts#L11-L48), hệ thống chạy hàm helper `recalculateCartTotal(cart)`:
     ```typescript
     const products = await productCol.find({ _id: { $in: productIds } }).toArray();
...
| **TC_CART_09** | [Unauthenticated] Không gửi Authorization Token $\to$ Reject 401 | Security (Auth Guard) | `POST /api/cart/add`<br>Headers: *None*<br>Payload: `{ productId: "prod_123", quantity: 1 }` | **401** | `{ error: "UNAUTHORIZED" }` | `it("TC-CART-09: [Unauthenticated] Không gửi Authorization Token -> Reject 401")` |
...
| 7 | **`if (quantity === 0)`** (`updateCart`: L161) | **`quantity == 0` $\to$ Xóa item (`splice`)** | **`quantity > 0` $\to$ Gán `quantity` mới** | **TC_CART_UPD_02** *(Xóa)* | **TC_CART_UPD_01** *(Sửa số lượng)* |
...
1. **Điểm mù 1: Xác thực cơ chế Anti-tampering Price (Kiểm tra nguồn gốc giá tiền)**
   - *Vấn đề của Blackbox*: Khi gửi payload có `price = 10`, Blackbox nhận về HTTP `200`. Nhưng Blackbox không thể biết được trong CSDL giỏ hàng đang lưu giá 10 hay giá 500,000 VND trừ khi có thêm một bước truy vấn DB.
   - *Cách Whitebox giải quyết*: Dùng Jest Mocking để kiểm tra xem `productCollection.findOne` có được gọi đúng ID và kết quả băm giá có được truyền vào `cart.totalPrice` hay không:
     ```typescript
...
2. **Điểm mù 2: Trạng thái giỏ hàng nội tại (New Cart vs Existing Cart vs Item Exists)**
   - *Vấn đề của Blackbox*: Để test nhánh "sản phẩm đã có trong giỏ hàng $\to$ cộng dồn số lượng", tester Blackbox phải gửi 2 request tuần tự. Nếu môi trường test dùng DB thật, việc dọn dẹp (clean up) sau mỗi test case rất chậm và dễ gây tình trạng test chạy chập chờn (flaky).
   - *Cách Whitebox giải quyết*: Sử dụng Mocking `mockCartCollection.findOne.mockResolvedValueOnce({ products: [{ productId: "prod_123", quantity: 2 }] })`, cho phép kiểm thử lập tức nhánh `index >= 0` chỉ với 1 lần gọi hàm.
3. **Điểm mù 3: Kích hoạt nhánh Xóa item khi `quantity === 0` trong `updateCart`**
   - *Vấn đề của Blackbox*: Người dùng chỉ biết gọi API cập nhật, không biết rằng bên dưới code sử dụng `splice` để giải phóng bộ nhớ thay vì lưu trữ bản ghi `quantity: 0`.
   - *Cách Whitebox giải quyết*: Viết test case truyền `quantity: 0`, mock giỏ hàng có sẵn 1 item và assert độ dài mảng `cart.products.length` giảm từ 1 về 0.
4. **Điểm mù 4: Kiểm thử khả năng chịu lỗi (Fault Tolerance & 500 Internal Server Error)**
   - *Cách Whitebox giải quyết*: Sử dụng `mockProductCollection.findOne.mockRejectedValue(new Error("MongoDB Connection Lost"))` để đưa Controller vào khối `catch (error)`, xác minh hệ thống luôn trả về JSON an toàn `{ error: "INTERNAL_SERVER_ERROR" }` thay vì làm sập tiến trình Node.js.
