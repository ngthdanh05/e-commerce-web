...
Theo lý thuyết BVA tiêu chuẩn trong tài liệu Chương 4, các trường dữ liệu giao hàng được kiểm thử qua **5 điểm biên chính**: $\text{Min}^-$ (Dưới ngưỡng), $\text{Min}$ (Cận dưới), $\text{Nom}$ (Điển hình), $\text{Max}$ (Cận trên) và $\text{Max}^+$ (Trên ngưỡng).

| Trường kiểm thử                  | Ngưỡng đặc tả     | Điểm biên ($\text{Min}^-$, $\text{Min}$, $\text{Nom}$, $\text{Max}$, $\text{Max}^+$)                                 | Dữ liệu đầu vào (Payload Sample)                                                                                                                                                   | Kết quả kỳ vọng                                                            | Ghi chú kỹ thuật                                             |
| :------------------------------- | :---------------- | :------------------------------------------------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------- | :----------------------------------------------------------- |
| **`phoneNumber`** (Độ dài chuỗi) | $[10, 10]$ số     | • $\text{Min}^- = 9$<br>• $\text{Min} = 10$<br>• $\text{Nom} = 10$<br>• $\text{Max} = 10$<br>• $\text{Max}^+ = 11$   | • `"091234567"` (9 số)<br>• `"0912345678"` (10 số)<br>• `"0381234567"` (10 số)<br>• `"0987654321"` (10 số)<br>• `"09123456789"` (11 số)                                            | • 400 Bad Request<br>• 200 OK<br>• 200 OK<br>• 200 OK<br>• 400 Bad Request | Regex: `^(03\|05\|07\|08\|09)\d{8}$` kiểm tra đúng 10 chữ số |
| **`address`** (Độ dài chuỗi)     | $[10, 200]$ ký tự | • $\text{Min}^- = 9$<br>• $\text{Min} = 10$<br>• $\text{Nom} = 50$<br>• $\text{Max} = 200$<br>• $\text{Max}^+ = 201$ | • `"Số 1 HCM"` (9 ký tự)<br>• `"1234567890"` (10 ký tự)<br>• `"123 Đường Lê Lợi, Quận 1, TP.HCM"` (36 ký tự)<br>• `'A'.repeat(200)` (200 ký tự)<br>• `'A'.repeat(201)` (201 ký tự) | • 400 Bad Request<br>• 200 OK<br>• 200 OK<br>• 200 OK<br>• 400 Bad Request | Zod Schema: `z.string().min(10).max(200)`                    |
...
4. **Băm HMAC-SHA512 với Secret Key**:
   - Chuỗi dữ liệu ký được tạo bằng thuật toán: **HMAC_SHA512(sorted_query_string, tmnSecret)**
5. **So sánh chữ ký**:
   ```typescript
   if (!secureHash || secureHash.toLowerCase() !== signData.toLowerCase()) {
     return res
       .status(400)
       .json({ success: false, errors: [{ message: "INVALID_CHECKSUM" }] });
   }
   ```
...
sequenceDiagram
    ...
    rect rgb(30, 50, 40)
    Note over Attacker, DB: Kịch bản 2: Chữ ký hợp lệ & Thành công (Valid Hash)
...
|  8  | **TC_CHK_ADD_01** _(Bổ sung)_    | Phủ nhánh thanh toán thất bại (`vnp_ResponseCode = 01`)    | L252-256 (`updateOne status: failed`, redirect failure)             |
|  9  | **TC_CHK_ADD_02** _(Bổ sung)_    | Phủ nhánh callback không tìm thấy mã đơn hàng trong DB     | L226-228 (`if (!order) return res.status(404)`)                     |
| 10  | **TC_CHK_ADD_03** _(Bổ sung)_    | Phủ nhánh cấu hình thiếu Secret Key trong môi trường       | L169-178 (`if (!tmnSecret) return res.status(500)`)                 |
| 11  | **TC_CHK_CATCH_ERR** _(Bổ sung)_ | Phủ các khối `catch (err)` bắt lỗi 500 bằng Mock DB Reject | L141-144 và L257-262 (`INTERNAL_SERVER_ERROR`)                      |
...
2. **Điểm mù 2: Kiểm thử Phục hồi lỗi & Giả lập môi trường Sandbox VNPay**
   - _Hạn chế của Blackbox_: Phụ thuộc vào cổng sandbox VNPay thật thường xuyên bị quá tải, lỗi mạng hoặc đòi hỏi tester phải nhập OTP ngân hàng bằng tay, khiến việc tự động hóa CI/CD trở nên bất khả thi.
   - _Cách Whitebox giải quyết_: Cô lập hoàn toàn VNPay SDK và MongoDB bằng Jest Mocking, mô phỏng chính xác cả hai kịch bản: VNPay trả về `vnp_ResponseCode = "00"` (Thành công) và mã lỗi khác (Thất bại), kiểm tra việc redirect 302 và cập nhật CSDL trong tích tắc mà không phụ thuộc Internet.
