...
describe("Cart schemas (BVA 5-point validation)", () => {
  it("Min- = 0 -> Reject 400 (Invalid boundary below min)", () => {
    expect(() => addToCartSchema.parse({ productId, quantity: 0 })).toThrow();
  });

  it("Min = 1 -> Accept 200 (Valid lower boundary)", () => {
  ...
  it("Nom = 50 -> Accept 200 (Valid nominal value)", () => {
    expect(addToCartSchema.parse({ productId, quantity: 50 })).toEqual({
      productId,
      quantity: 50,
    });
  });

  it("Max = 99 -> Accept 200 (Valid upper boundary)", () => {
  ...
  it("Max+ = 100 -> Reject 400 (Invalid boundary above max)", () => {
    expect(() => addToCartSchema.parse({ productId, quantity: 100 })).toThrow();
  });

  it("rejects invalid values (floats, negative, empty string)", () => {
