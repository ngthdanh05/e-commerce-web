...
export default function ProductReview() {
  const [review, setReview] = useState<ApiReview[]>([]);
  ...
  const fetchReview = async (page: number = currentPage) => {
    setIsLoading(true);
    try {
      const res = await httpRequest(
        `/orders?status=success&page=${page}&limit=${limit}`
      );
  ...
  if (isLoading) {
    return (
      <div className="max-w-5xl mx-auto bg-white rounded-2xl p-8 ml-4 shadow-md">
        <h2 className="text-2xl font-bold mb-6">Chờ đánh giá</h2>
        <p>Đang tải...</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-5xl mx-auto bg-white rounded-2xl p-8 ml-4 shadow-md">
      ...
      {review.length === 0 ? (
        <p>Bạn chưa có đơn hàng nào cần đánh giá.</p>
      ) : (
        <div className="space-y-4">
          {review.map((tx, index) => {
            const { id, amount, createdAt, products } = tx;
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  ...
                  <div className="flex flex-col sm:items-end gap-2">
                    ...
                    <span className="inline-flex items-center gap-1 px-3 py-1 text-xs sm:text-sm font-medium rounded-full bg-green-100 text-green-700">
                      <FontAwesomeIcon icon={faCircleCheck} />
                      Đã thanh toán
                    </span>
                    <Link
                      to={`/account/productReview/${id}`}
                      className="w-full flex justify-center mt-2 bg-[#ff3b5c] rounded-md"
                    >
                      <span className="text-white text-right font-semibold p-1">
                        Viết đánh giá
                      </span>
                    </Link>
                  </div>
